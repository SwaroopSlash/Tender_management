...
def main():
    ...
    if 'user_api_key' not in st.session_state:
        st.session_state.user_api_key = ""  # Initialize the API key
    
    # Initialize session state
    if 'qa_system' not in st.session_state:
        # Default to using the secret API key if no user-provided key is available
    ...
    with st.sidebar:
        ...
        # Update the QA system with the new API key if it has changed
        if st.session_state.user_api_key and st.session_state.user_api_key != st.session_state.qa_system.google_api_key:
            st.session_state.qa_system = PDFQuestionAnswering(st.session_state.user_api_key)
            st.rerun()
        ...
        user_api_key = st.text_input("Enter your Gemini API Key:", type="password", value=st.session_state.get("user_api_key", ""))
